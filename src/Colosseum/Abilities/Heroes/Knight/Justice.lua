...
function AbilityTrigger_Knight_Justice_Actions()
    ...
    TimerStart(timer, 0.25, true, function()
        ...
        local projectileData = FireProjectile_PointHeightToPoint(startLoc, 1500, targetLoc, "war3mapImported\\HauntedSwordAttach.mdl", 20.0, 0, function(callbackData)
            ...
            local thunderEffect = CreateEffectAtPoint(targetLoc, "Abilities\\Spells\\Human\\Thunderclap\\ThunderClapCaster.mdl", 4.0)
            BlzSetSpecialEffectScale(thunderEffect, 2.2)
            BlzSetSpecialEffectTimeScale(thunderEffect, 0.45)
            local monsoonEffect = CreateEffectAtPoint(targetLoc, "Abilities\\Spells\\Other\\Monsoon\\MonsoonBoltTarget.mdl", 2.00)
            local flameStrikeEffect = CreateEffectAtPoint(targetLoc, "war3mapImported\\Flamestrike Starfire I.mdl", 4.0)
